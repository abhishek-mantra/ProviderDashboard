...
import { ArrowLeft, Shield, CheckCircle, XCircle, Loader2, AlertTriangle, Clock } from "lucide-react";
import { MOCK_CREDENTIAL_STATUS, findPayerForClient } from "../types/claims";
...
export function MantraEligibility() {
  ...
  const resolvedPayer = useMemo(() => findPayerForClient(client), [client]);
  const credentialStatus = resolvedPayer
    ? MOCK_CREDENTIAL_STATUS[resolvedPayer.id] ?? "not_credentialed"
    : "not_credentialed";
  ...
  const navigateToSessions = () => {
    navigate(`/claims/new/${clientId}/mantra/sessions`, {
      state: { payer: resolvedPayer, eligibilityConfirmed: true },
    });
  };
      ...
      <div className="flex items-center gap-2 md:gap-4 pb-3 md:pb-4">
        ...
        <div>
          ...
          <p className="text-xs md:text-sm text-gray-600 dark:text-gray-400">
            {isCA ? "Canada: TELUS Health eClaims" : labels.title}
          </p>
        </div>
      </div>
        ...
        <div className="p-4 md:p-8 space-y-6">
            ...
            <div className="flex items-center justify-between p-4 border border-gray-200 dark:border-gray-700 rounded-xl">
              <div>
                <p className="font-semibold text-gray-900 dark:text-white">
                  {resolvedPayer?.name || "No payer on file"}
                </p>
                <p className="text-sm text-gray-500 dark:text-gray-400">
                  {resolvedPayer?.intermediaryName || "—"}
                </p>
              </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-red-600 dark:text-red-300 mb-3">
                    You are not credentialed with {resolvedPayer?.name || "this payer"}. Please complete credentialing before submitting claims.
                  </p>
                  <button
                    onClick={() => navigate("/claims/credential-status")}
          ...
          {credentialStatus === "pending" && (
            <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-xl p-4 md:p-6">
              <div className="flex items-start gap-3">
                <Clock className="size-5 text-yellow-600 dark:text-yellow-400 mt-0.5 flex-shrink-0" />
                <div>
                  <p className="font-semibold text-yellow-700 dark:text-yellow-400 mb-1">
                    Credentialing pending
                  </p>
                  <p className="text-sm text-yellow-600 dark:text-yellow-300">
                    Your credentialing application with {resolvedPayer?.name || "this payer"} is still being processed.
                  </p>
                </div>
              </div>
            </div>
              ...
              {isCA ? (
                <div>
                  <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">
                    Canada (TELUS eClaims): Predetermination check is done on the claim submission screen. Proceed directly.
                  </p>
                  <div className="flex justify-end">
                    <button
                      onClick={navigateToSessions}
                      className="px-6 py-2.5 bg-[#00c0ff] hover:bg-[#0090c0] text-white rounded-lg font-medium transition-all shadow-lg"
                    >
                    ...
                    <div className="mt-4 p-4 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg">
                      <div className="flex items-start gap-3">
                        ...
                        <div className="w-full">
                          ...
                          <button
                            onClick={navigateToSessions}
                            className="mt-3 px-6 py-2 bg-[#00c0ff] hover:bg-[#0090c0] text-white rounded-lg font-medium transition-all shadow-lg"
                          >
                            Continue to Sessions
                          </button>
                        </div>
